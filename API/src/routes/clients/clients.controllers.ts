...
const postDebtor = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const { debtor = '', telephone = 0, patients } = req.body as Debtor;
    const mainDebtor: DebtorPatients = patients!.filter(
      (ele) => ele.name === undefined || ele.main
    )![0];
    const patientsFilter: DebtorPatients[] = patients!.filter(
      (ele) => !ele.main
    );
    const debtorUUID = randomUUID();
    const createDebtor = await clients
      .create(
        {
          id: debtorUUID,
          debtor,
      ...
      .catch((err: ErrorEvent) => {
        log.error(
          `Error in clients postDebtor createDebtor, error: ${err.message}`
        );
        throw new Error(
          `Error in clients postDebtor createDebtor, error: ${err}`
        );
      });
    ...
    patientsFilter.forEach(async (pat) => {
      await clients
        ...
        .catch((err: ErrorEvent) => {
          log.error(
            `Error in clients postDebtor create patient, error: ${err.message}`
          );
          throw new Error(
            `Error in clients postDebtor create patient, error: ${err}`
          );
        });
    });
...
const getDebtor = async (
  ...
  try {
    ...
    const patients = await clients
      ...
      .catch((err: ErrorEvent) => {
        log.error(
          `Error in clients getDebtor, patients, error: ${err.message}`
        );
...
const deleteClient = async (
  ...
  try {
    const { clientID } = req.query;
    const makeInActive = await clients.update(
      { active: false },
      {
        where: {
          id: clientID as string,
...
export { getDebtorsAll, postDebtor, getDebtor, putClient, deleteClient };
